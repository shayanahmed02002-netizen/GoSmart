          ...
          <div key={m.id} className={`flex gap-2 ${m.from === 'user' ? 'flex-row-reverse' : ''}`}>
            <div
              className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 ${
                m.from === 'user' ? 'bg-primary text-white' : 'bg-canvas border border-line text-ink'
              }`}
            >
            ...
            <div className={`max-w-[80%] ${m.from === 'user' ? 'items-end' : 'items-start'} flex flex-col`}>
              <div
                className={`rounded-xl2 px-3.5 py-2.5 text-sm whitespace-pre-wrap ${
                  m.from === 'user' ? 'bg-primary text-white' : 'bg-canvas text-ink border border-line'
                }`}
              >
                {m.text}
              </div>
