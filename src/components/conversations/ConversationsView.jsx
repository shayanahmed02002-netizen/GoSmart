...
export default function ConversationsView() {
  ...
  // Live updates for whichever conversation is open. Re-subscribes whenever
  // the active thread changes, and tears the channel down on cleanup.
  useEffect(() => {
    if (!activeId) return

    const unsubscribe = dataService.subscribeToConversation(activeId, {
      onMessage: (msg) => {
        setConvos((prev) => prev.map((c) => {
          if (c.id !== activeId) return c
          // Skip messages we already added optimistically (matched by text +
          // role, since the temp row has a local 'tmp' id, not the real one).
          const alreadyHave = c.messages.some((m) => m.id === msg.id
            || (m.id === 'tmp' && m.from === msg.from && m.text === msg.text))
          if (alreadyHave) {
            return { ...c, messages: c.messages.map((m) => (m.id === 'tmp' ? msg : m)) }
          }
          return { ...c, messages: [...c.messages, msg] }
        }))
      },
      onConversationUpdate: (patch) => {
        setConvos((prev) => prev.map((c) => (c.id === patch.id ? { ...c, ...patch } : c)))
      },
    })
